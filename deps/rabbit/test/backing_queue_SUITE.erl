...
groups() ->
    ...
    [
     {backing_queue_tests, [], [
          ...
          msg_store_gc_stop_timeout_mid_callback,
...
%% the msg_store recovers cleanly because terminate kills the GC after a bounded
%% timeout and proceeds to write recovery files.
msg_store_gc_stop_timeout_mid_callback(Config) ->
    rabbit_ct_broker_helpers:setup_meck(Config),
    passed = rabbit_ct_broker_helpers:rpc(Config, 0,
      ?MODULE, msg_store_gc_stop_timeout_mid_callback1, [Config]).

msg_store_gc_stop_timeout_mid_callback1(_Config) ->
    GenRef = fun() -> make_ref() end,
    restart_msg_store_empty(),
    ...
    ok = meck:expect(rabbit_msg_store, compact_file,
                     fun(_, _) ->
                         ...
                         %% completes. The GC will be killed by stop_gc/2.
    ...
    %% Set a short shutdown timeout so the GC stop times out quickly.
    %% The GC timeout is max(ShutdownTimeout - 60000, 5000), so any value
    %% below 65000 clamps to 5000ms. Use 30000ms to give the supervisor
    %% enough headroom to wait for terminate to write recovery files after
    %% killing the GC.
    ok = application:set_env(rabbit, msg_store_shutdown_timeout, 30_000),

    %% Terminate the persistent store via the supervisor. The GC is blocked
    %% mid-callback so the stop call will time out, the GC will be killed,
    %% and terminate will proceed to write recovery files.
    {ok, VHostSup} = rabbit_vhost_sup_sup:get_vhost_sup(?VHOST),
    ok = supervisor:terminate_child(VHostSup, ?PERSISTENT_MSG_STORE),
    ...
    %% The store should report a clean recovery because the fix kills the
    %% unresponsive GC and proceeds to write recovery files.
    true = rabbit_vhost_msg_store:successfully_recovered_state(
                ?VHOST, ?PERSISTENT_MSG_STORE),
